...
function dynamickyModelujRodinu(simulace = 0) {
      ...
      //$('.filter-graf a').removeClass('active');
     ...
     /*$('.highcharts-legend .highcharts-legend-item').each(function(index){
     		...
     		$('.filter-graf a').removeClass('d-none');

      for ( var i = 0, l = 18; i < l; i++ ) {

        var class_high = 'highcharts-series-' + i;
        console.log(class_high);
        if($('.highcharts-legend .highcharts-legend-item').hasClass(class_high)){
          $(this).hasClass(class_high)
        }else{
          var test = '.filter-graf .' + class_high;
          $('.filter-graf .' + class_high).addClass('d-none');
